...
void Node::checkForOneStep() {
	for (auto const &pair : connections) {
		...
		for (auto const &pair2 : connections) {
			...
			for (int k = 0; k < compare->getSize() || k < to_compare->getSize(); k++) {
				if (compare->getSize() == k < to_compare->getSize()) {
					if (compare->returnAsBoolVec().at(k) != to_compare->returnAsBoolVec().at(k))
						differences++;
				}
				else if (compare->getSize() > to_compare->getSize()) {
					if (compare->returnAsBoolVec().at(k) != 0)
						differences++;
				}
				else if (compare->getSize() < to_compare->getSize()) {
					if (to_compare->returnAsBoolVec().at(k) != 0)
						differences++;
				}
				if (differences == 1) {
...
void Node::setWeight(short k) {
    weight = 0;
    for(int i = 2; i >= k; i--)
        weight += neighbours[i].size()*std::pow(getNumNodes(), 2-i);
}
